...
class MYADDON_OT_export_scene(bpy.types.Operator, bpy_extras.io_utils.ExportHelper):
    ...
    def parse_scene_recursive(self, file, object, level):
        ...
        #トランスフォーム情報を表示（C++で読み込みやすいように空白区切りに変更）
        self.write_and_print(file, indent + "T %f %f %f" % (trans.x, trans.y, trans.z) )
        self.write_and_print(file, indent + "R %f %f %f" % (rot.x, rot.y, rot.z) )
        self.write_and_print(file, indent + "S %f %f %f" % (scale.x, scale.y, scale.z) )
        
        #カスタムプロパティ'file_name'が存在すれば書き出す
        if "file_name" in object:
            self.write_and_print(file, indent + "N %s" % object["file_name"])
        
        #カスタムプロパティ'collider'が存在すれば書き出す
        if "collider" in object:
            self.write_and_print(file, indent + "C %s" % object["collider"])
            # 中心座標とサイズを取得して出力（.xではなく[0]を使います）
    ...
    def parse_scene_recursive_json(self, data_parent, object, level):
        ...
        if "collider" in object:
            ...
            # JSONエンコーダーでエラーにならないよう、to_list()でリストに変換しておく
    ...
    def export_json(self):
        ...
        #ファイルをテキスト形式で書き出し用にオープン（文字コードはUTF-8を指定）
